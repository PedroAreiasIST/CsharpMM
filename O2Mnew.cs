...
/// </summary>
/// <remarks>
/// THREAD SAFETY: This class is NOT thread-safe. Callers must provide external synchronization
/// if instances are accessed from multiple threads concurrently. The internal cache assumes 
/// single-threaded access patterns.
/// 
/// PERFORMANCE: This class minimizes validation for performance. Methods assume valid input.
/// Callers are responsible for ensuring indices are in range and data is valid.
/// </remarks>
public class O2M : IComparable<O2M>, IEquatable<O2M>, ICloneable
{
    ...
    /// <summary>
    /// Parallelization threshold for operations. Default is 4096.
    /// Operations with fewer elements than this value execute sequentially.
    /// </summary>
    public static int ParallelizationThreshold { get; set; } = 4096;
    ...
    /// </summary>
    /// <remarks>
    /// Performs full deep copy of all adjacency lists, which may be expensive for large structures.
    /// </remarks>
    public virtual object Clone()
    {
        var clonedO2m = new O2M(_adjacencies.Count);
        foreach (var row in _adjacencies) 
            clonedO2m._adjacencies.Add(new List<int>(row));
        return clonedO2m;
    }
    ...
    public bool Equals(O2M? other)
    {
        ...
        for (var i = 0; i < a.Count; i++)
        {
            ...
            if (!CollectionsMarshal.AsSpan(ar).SequenceEqual(CollectionsMarshal.AsSpan(br))) 
                return false;
        }
    ...
    {
        return workItems >= ParallelizationThreshold;
    }
    ...
    public static List<List<int>> GetCliques(O2M elementsToNodes, O2M nodesToElements)
    {
        ...
        if (ShouldParallelize(elementCount))
            Parallel.For(0, elementCount, elementId => 
            { 
        ...
        static void BuildCliqueForElement(List<List<int>> e2nAdj, int elementId, 
            Dictionary<int, int> map, List<int>[] outArr)
        {
    ...
    public List<int> GetTopOrder()
    {
        ...
        while (q.TryDequeue(out var u))
        {
            ...
            if (u < Count)
            {
                ...
                {
                    var v = span[i];
                    if ((uint)v < (uint)nodeCount && --inDeg[v] == 0) 
                        q.Enqueue(v);
                }
            }
        }
    ...
    /// <summary>
    /// Reorders elements according to the specified permutation mapping.
    /// </summary>
    /// <param name="oldToNewElementMap">
    /// Mapping where oldToNewElementMap[oldIndex] = newIndex. 
    /// Must be a valid permutation of [0..Count-1].
    /// </param>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the mapping is not a valid permutation.
    /// </exception>
    [MethodImpl(AggressiveOptimization | AggressiveInlining)]
    [SkipLocalsInit]
        ...
        for (var old = 0; old < n; old++)
        {
            ...
            if ((uint)nw >= (uint)n || seen[nw])
            {
                throw new InvalidOperationException(
                    "PermuteElements requires a valid permutation mapping. " +
                    "Use CompressElements for non-permutation remapping.");
            }
            seen[nw] = true;
        }
    ...
    public virtual void PermuteNodes(List<int> oldToNewNodeMap)
    {
        ...
        for (var i = 0; i < lists.Count; i++)
        {
            ...
            {
                var oldIdx = span[j];
                if ((uint)oldIdx < (uint)map.Length) 
                    span[j] = map[oldIdx];
            }
        }
    ...
    public static List<List<int>> GetElementPositions(O2M elementsToNodes, O2M nodesToElements)
    {
        ...
        for (var e = 0; e < elemCount; e++) 
            elemPos.Add(new List<int>(new int[elementsToNodes._adjacencies[e].Count]));
    ...
    public O2M Transpose()
    {
        ...
        Parallel.For(0, chunkCount, c =>
        {
            ...
            for (var i = start; i < end; i++)
            {
                ...
                foreach (var targetNode in adj) 
                    localCounts[targetNode]++;
            }
        });
        ...
        for (var i = 0; i < targetCount; i++) 
            result._adjacencies.Add(adjacencyLists[i] ?? new List<int>(0));
    ...
    [MethodImpl(AggressiveOptimization | AggressiveInlining)]
    [SkipLocalsInit]
    private static List<List<int>> PerformSymbolicMultiplicationUnchecked(
        List<List<int>> aRows, List<List<int>> bRows)
    {
    ...
    [MethodImpl(AggressiveOptimization | AggressiveInlining)]
    [SkipLocalsInit]
    private static List<List<int>> PerformSymbolicMultiplicationChecked(
        List<List<int>> aRows, List<List<int>> bRows)
    {
        var rowCount = aRows.Count;
        var productRows = new List<int>[rowCount];
